...
/**
 * @brief calculate the return value of the last winning bingo field
 * 
 * @return int 
 */
int main()
{
    ...
    vector<int32_t>number_stream;
    vector<BingoFeld>bingo_feld_vector;

    //get numbers from input file
    read_number_stream(number_stream, file);
    //read all the bingo fields and safe them 
    while(!file.eof())
        bingo_feld_vector.push_back(BingoFeld(file));
    file.close();

    //iterate through all the input numbers
    for(auto& number: number_stream)
    {
        //iterate though all bingo fields
        auto it = bingo_feld_vector.begin();       
        while(it != bingo_feld_vector.end())
        {
            //get return value
            return_value = it->mark_number(number);
            //if field has won
            if(return_value != -1)
            {
                //and last bingo fields hasnt been reached yet
                if(bingo_feld_vector.size() > 1)
                    //delete field. Iterator now points to next element after the deleted one
                    it = bingo_feld_vector.erase(it);
                //if last field has been reached    
                else 
                {
                    //output result and terminate programm
                    cout << "Result: " << return_value << endl;
                    return 0;
                }
            }
            //increase iterator only if nothing gets deleted
            else
                it++;
        }
    }
